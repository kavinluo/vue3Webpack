...
multiple2 (rule, value, callback) {
  ...
  float1 (min = 0, max = 100) {
    return (rule, value, callback) => {
      if (value == '') callback()
      if (/^\d+\.?\d*$/m.test(value)) { /* empty */ } else {
  ...
  float2 (min = 0, max = 100) {
    return (rule, value, callback) => {
      if (value == '') callback()
      if (/^\d+.?\d{0,2}$/m.test(value)) { /* empty */ } else {
        callback(new Error('最多两位小数的数字'))
      }
      if (+value >= +min && +value <= +max) {
        callback()
  ...
  _illegalChar:  function (value, callback, message){
    if (!(/[^\u4e00-\u9fa5\w\s，。、；‘’“”《》？——－+-、~·！\]_（）()【】：]/gm.test(
    value))) {
      console.log('', )
    } else {
      callback(message);
